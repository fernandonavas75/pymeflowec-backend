...
const service = require('../services/expensePayment.service');
const { parsePagination, paginatedResponse } = require('../utils/pagination');

const list = async (req, res, next) => {
  try {
    const { page, limit, offset } = parsePagination(req.query);
    const expenseId = req.query.expense_id ? parseInt(req.query.expense_id, 10) : undefined;
    const result = await service.list(req.user.company_id, { expenseId, limit, offset });
