...
const cancel = async (req, res, next) => {
  try {
    const data = await service.cancel(req.params.id, req.user.organization_id);
    res.status(200).json({ success: true, data });
  } catch (err) { next(err); }
};
...
const listCategories = async (req, res, next) => {
  try {
    const data = await service.listCategories(req.user.organization_id);
    res.status(200).json({ success: true, data });
  } catch (err) { next(err); }
};
...
const createCategory = async (req, res, next) => {
  try {
    const data = await service.createCategory(req.body, req.user.organization_id);
    res.status(201).json({ success: true, data });
  } catch (err) { next(err); }
};
